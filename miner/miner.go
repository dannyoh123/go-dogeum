...
type Config struct {
	Dogebase  common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.SetDogebase(miner.coinbase)
					miner.worker.start()
				}
...
func (miner *Miner) SetDogebase(addr common.Address) {
	miner.coinbase = addr
	miner.worker.setDogebase(addr)
}
...
// given parameters. Regardless of whdoge the generation is successful or not,
