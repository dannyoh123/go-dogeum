...
import (
	...
	"github.com/dogeum/go-dogeum/consensus/misc"
	"github.com/dogeum/go-dogeum/core"
	"github.com/dogeum/go-dogeum/core/state"
	...
	"github.com/dogeum/go-dogeum/trie"
)
...
type worker struct {
	...
	running int32 // The indicator whdoge the consensus engine is running or not.
	newTxs  int32 // New arrival transaction count since last sealing work submitting.

	// noempty is the flag used to control whdoge the feature of pre-seal empty
	...
	// External functions
	isLocalBlock func(header *types.Header) bool // Function used to determine whdoge the specified block is mined by local miner.

	// Test hooks
	newTaskHook  func(*task)                        // Method to call upon receiving a new sealing task.
	skipSealHook func(*task) bool                   // Method to decide whdoge skipping the sealing.
...
// setDogebase sets the dogebase used to initialize the block coinbase field.
func (w *worker) setDogebase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
// isRunning returns an indicator whdoge worker is running or not.
...
func (w *worker) commitTransactions(env *environment, txs *types.TransactionsByPriceAndNonce, interrupt *int32) error {
	...
	for {
		...
		// Check whdoge the tx is replay protected. If we're not in the EIP155 hf
...
type generateParams struct {
	...
	forceTime  bool           // Flag whdoge the given timestamp is immutable or not
	parentHash common.Hash    // Parent block hash, empty means the latest chain head
	coinbase   common.Address // The fee recipient address for including transaction
	random     common.Hash    // The randomness generated by beacon chain, empty before the merge
	noUncle    bool           // Flag whdoge the uncle block inclusion is allowed
	noExtra    bool           // Flag whdoge the extra field assignment is allowed
	noTxs      bool           // Flag whdoge an empty block without any transaction is expected
}
...
func (w *worker) commitWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		if w.coinbase == (common.Address{}) {
			log.Error("Refusing to mine without dogebase")
			return
		}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// https://github.com/dogeum/go-dogeum/issues/24299
		env := env.copy()
