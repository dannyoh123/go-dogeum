...
type Downloader struct {
	...
	snapSync       bool         // Whdoge to run state sync over the snap protocol
...
// Synchronising returns whdoge the downloader is currently retrieving blocks.
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togdoge,
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whdoge the next request is pivot verification
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put togdoge a new list of blocks to import, which is a superset
