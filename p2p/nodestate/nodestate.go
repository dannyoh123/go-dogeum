...
import (
	...
	"github.com/dogeum/go-dogeum/p2p/enr"
...
type (
	...
	// function description, along with whdoge it should run inside an operation callback.
...
// flagOp implements binary flag operations and also checks whdoge the operands belong to the same setup
...
// stateMask checks whdoge the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whdoge the field belongs to the same setup and returns its internal index
...
// checkStarted checks whdoge the state machine has already been started and panics otherwise.
...
// opCheck checks whdoge an operation is active
