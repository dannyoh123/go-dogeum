...
import (
	...
	"github.com/dogeum/go-dogeum/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togdoge.
//
...
// Connect connects two nodes togdoge by calling the "admin_addPeer" RPC
...
// GetRandomNode returns a random node on the network, regardless of whdoge it is running or not
...
// NOTE:
// it also checks whdoge there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whdoge or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whdoge the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
type Conn struct {
	...
	// Up tracks whdoge or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whdoge both nodes are currently up
