...
// Remove deletes a transaction from the maintained map, returning whdoge the
...
type txList struct {
	strict bool         // Whdoge nonces are strictly continuous or not
...
// Overlaps returns whdoge the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whdoge the
...
// Remove deletes a transaction from the maintained list, returning whdoge the
...
// Empty returns whdoge the list of transactions is empty or not.
...
// Underpriced checks whdoge a transaction is cheaper than (or as cheap as) the
...
// underpricedFor checks whdoge a transaction is cheaper than (or as cheap as) the
