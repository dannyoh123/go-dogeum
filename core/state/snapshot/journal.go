...
type journalGenerator struct {
	// Indicator that whdoge the database was in progress of being wiped.
	// It's deprecated but keep it here for background compatibility.
	Wiping bool

	Done     bool // Whdoge the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func ParseGeneratorStatus(generatorBlob []byte) string {
	...
	// Figure out whdoge we're after or within an account
	var m string
...
func loadSnapshot(diskdb ethdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	if !generator.Done {
		// Whdoge or not wiping was in progress, load any generator progress too
