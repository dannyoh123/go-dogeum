...
import (
	...
	"github.com/dogeum/go-dogeum/common/bitutil"
...
type Matcher struct {
	...
	running uint32 // Atomic flag whdoge a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togdoge.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togdoge
				var orVector []byte
...
// the requested retrieval queue to be serviced togdoge with other sessions.
//
