...
import (
	...
	"github.com/dogeum/go-dogeum/common/mclock"
	"github.com/dogeum/go-dogeum/common/prque"
	"github.com/dogeum/go-dogeum/consensus"
	"github.com/dogeum/go-dogeum/core/rawdb"
	"github.com/dogeum/go-dogeum/core/state"
	"github.com/dogeum/go-dogeum/core/state/snapshot"
	...
	"github.com/dogeum/go-dogeum/internal/syncx"
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whdoge to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whdoge to disable trie write caching and GC altogdoge (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Whdoge to store preimage of trie key to the disk
...
// empty returns an indicator whdoge the blockchain is empty.
...
// SetHead rewinds the local chain to a new head. Depending on whdoge the node
...
// persistent disk layer. Depending on whdoge the node was fast synced or full, and
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db ethdb.KeyValueWriter, header *types.Header) (uint64, bool) {
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() <= currentBlock.NumberU64() {
			...
			if newHeadBlock == nil {
				...
				// root hash
				beyondRoot := (root == common.Hash{}) // Flag whdoge we're beyond the requested root (no root, always true)
	...
	// touching the header chain altogdoge, unless the freezer is broken
	if repair {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	// and returns an indicator whdoge the inserted blocks are canonical.
...
//
// The verify parameter can be used to fine tune whdoge nonce verification
