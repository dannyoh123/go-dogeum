...
// checking whdoge the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // whdoge the HTTP server should be reachable at all
		wantHandlers  bool // whdoge RegisterHandler handlers should be accessible
		wantRPC       bool // whdoge JSON-RPC/HTTP should be accessible
		wantWS        bool // whdoge JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks whdoge the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks whdoge JSON-RPC works against the given URL.
