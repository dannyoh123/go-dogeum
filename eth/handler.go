...
import (
	...
	"github.com/dogeum/go-dogeum/eth/fetcher"
...
type txPool interface {
	// Has returns an indicator whdoge txpool has a transaction
...
type handlerConfig struct {
	...
	Sync           downloader.SyncMode       // Whdoge to snap or full sync
...
type handler struct {
	...
	snapSync  uint32 // Flag whdoge snap sync is enabled (gets disabled if we already have blocks)
	acceptTxs uint32 // Flag whdoge we're considered synchronised (enables transaction processing)
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// out a way yet where nodes can decide unilaterally whdoge the network is new
		...
		if h.merger.TDDReached() {
			...
			// pow blocks. Spec: https://github.com/dogeum/EIPs/blob/master/EIPS/eip-3675.md#halt-the-importing-of-pow-blocks
