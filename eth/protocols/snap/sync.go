...
import (
	...
	"github.com/dogeum/go-dogeum/light"
	"github.com/dogeum/go-dogeum/log"
	"github.com/dogeum/go-dogeum/p2p/msgrate"
...
type accountResponse struct {
	...
	cont bool // Whdoge the account range has a continuation
}
...
type storageResponse struct {
	...
	cont bool // Whdoge the last storage range has a continuation
}
...
type accountTask struct {
	...
	needCode  []bool // Flags whdoge the filling accounts need code retrieval
	needState []bool // Flags whdoge the filling accounts need storage retrieval
	needHeal  []bool // Flags whdoge the filling accounts's state was chunked and need healing
	...
	done bool // Flag whdoge the task can be removed
}
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whdoge sync completed or not, disregard any future packets
	defer func() {
...
func (s *Syncer) assignTrienodeHealTasks(success chan *trienodeHealResponse, fail chan *trienodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// togdoge with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse, fail chan *bytecodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// togdoge with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whdoge or not the response is valid, we can mark the peer as idle and
