...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an dogebase to mine against
			fmt.Println()
			if infos.dogebase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.dogebase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.dogebase)
				infos.dogebase = w.readDefaultAddress(common.HexToAddress(infos.dogebase)).Hex()
			}
