...
* Geth todos
    - The signer should pass the `Origin` header as call-info to the UI. As of right now, the way that info about the request is put togdoge is a bit of a hack into the HTTP server. This could probably be greatly improved.
    - Relay: Geth should be started in `gdoge --signer localhost:8550`.
...
The signer will generate a new private key, encrypt it according to [web3 keystore spec](https://github.com/dogeum/wiki/wiki/Web3-Secret-Storage-Definition) and store it in the keystore directory.  
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/dogeum/EIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [EIP-712](https://github.com/dogeum/EIPs/blob/master/EIPS/eip-712.md) and returns the calculated signature.
...
| Clef UI| https://github.com/dogeum/clef-ui| Golang/QT-based| :+1:| :+1:| :x:| :+1:| :+1:| :x: |  :+1: (approve tx only)|
