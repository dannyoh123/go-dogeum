...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here because we don't know whdoge
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whdoge
...
// Contains returns whdoge an account is part of this particular wallet or not.
...
type Session struct {
	...
	verified bool                  // Whdoge the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whdoge the card has been initialized with a private key
}
